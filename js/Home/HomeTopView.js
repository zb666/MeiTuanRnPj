...
export default class HomeTopView extends Component<Props> {
    ...
    // == 一般相等(抽象相等)，=== 代表的是严格相等
    //比如 var strA =‘love' var strB = new String('love')
    //第一种情况会先尝试各种情况，检测两者在某些情况之下是否相等，但是第二种则不会 直接进行报错。
    renderIndicator() {
        var indicatorArr = [];
    ...
    render() {
        //SctrollView 可以开始indicator 横向ScroolView+指示器 + 当前页面的计算 根据选中的界面判断当前的红色标签
        return (
